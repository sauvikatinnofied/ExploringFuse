// This file was generated based on '/usr/local/share/uno/Packages/UnoCore/0.24.3/Targets/CPlusPlus/Source/Uno/ObjectModel.h'.
...
struct uObject
{
    union { uType* __type; uType* __obj_type; };
    union { uWeakObject* __weakptr; uWeakObject* __obj_weak; };
#ifdef DEBUG_ARC
    size_t __size;
    size_t __id;
#endif
...
{
    union { const char* FullName; const char* TypeName; };
    uint32_t Type;
    uint16_t Flags;
    uint16_t State;
    size_t TypeSize;
...
struct uThrowable : public std::exception
{
    ...
    static XLI_NORETURN void ThrowInvalidOperation(const char* func, int line);
...
#define U_THROW_IOE() uThrowable::ThrowInvalidOperation(XLI_FUNCTION, XLI_LINE)
...
template<class T>
uTPtr uConstrain(uType* type, const T& value) {
    U_ASSERT(type && type->ValueSize == sizeof(T));
    return U_IS_VALUE(type)
        ? const_cast<T*>(&value)
        : *(void**)&value;
}
inline uTPtr uUnboxAny(uType* type, uObject* object) {
    U_ASSERT(type);
    return U_IS_VALUE(type)
...
// Deprecated
inline uArray* uNewArray(uType* elementType, int length, const void* optionalData = NULL) {
    return uArray::New(elementType->Array(), length, optionalData);
}
inline uString* uNewStringUtf8(const char* mutf8, int length) {
    return uString::Utf8(mutf8, length);
}
inline uString* uNewStringUtf8(const char* mutf8) {
    return uString::Utf8(mutf8);
}
inline const char* uStringToCStr(uString* str) {
    return uAllocCStr(str);
}
