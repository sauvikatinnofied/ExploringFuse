// This file was generated based on '/usr/local/share/uno/Packages/Fuse.Themes/0.24.6/.uno/$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#pragma once
#include <Fuse.Controls.Path.h>
#include <Uno.UX.ITemplate.h>
#include <Uno.UX.Template-1.h>
namespace g{namespace Fuse{namespace Themes{struct GraphicsStyle;}}}
namespace g{namespace Fuse{namespace Themes{struct GraphicsStyle__Template6;}}}

namespace g{
namespace Fuse{
namespace Themes{

// public partial sealed class GraphicsStyle.Template6 :102
// {
::g::Uno::UX::Template_type* GraphicsStyle__Template6_typeof();
void GraphicsStyle__Template6__ctor_1_fn(GraphicsStyle__Template6* __this, ::g::Fuse::Themes::GraphicsStyle* parent);
void GraphicsStyle__Template6__New1_fn(::g::Fuse::Themes::GraphicsStyle* parent, GraphicsStyle__Template6** __retval);
void GraphicsStyle__Template6__OnApply_fn(GraphicsStyle__Template6* __this, ::g::Fuse::Controls::Path* self);

struct GraphicsStyle__Template6 : ::g::Uno::UX::Template
{
    uStrong< ::g::Fuse::Themes::GraphicsStyle*> __parent1;

    void ctor_1(::g::Fuse::Themes::GraphicsStyle* parent);
    static GraphicsStyle__Template6* New1(::g::Fuse::Themes::GraphicsStyle* parent);
};
// }

}}} // ::g::Fuse::Themes
